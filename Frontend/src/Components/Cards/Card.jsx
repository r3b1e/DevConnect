...
const Card = ({ user, getConnection, interested }) => {
  ...
  const acceptRequest = async () => {
    try {
      ...
      dispatch(removeInterested(user._id))
      dispatch(addConnected([user]));
      getConnection();
    } catch (error) {
      console.error(error);
    }
  ...
  const rejectRequest = async () => {
    try {
      const res = await axios.put(
        `http://localhost:8080/api/request/review/rejected/${user._id}`,
        {},
        {
          withCredentials: true,
        }
      ...
      <div className="flex flex-col sm:flex-row items-start gap-6">
        ...
        <div className="flex-shrink-0">
          <img
            ...
            onError={(e) => {
              e.currentTarget.src =
                "https://i.pinimg.com/736x/92/44/49/9244499af3cbdc9ce5a559ddab217505.jpg"; // swap in fallback if the URL 404s
            }}
          />
