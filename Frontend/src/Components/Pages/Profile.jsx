...
export default function Connection({ profileId = "1", isOwnProfile = true }) {
  ...
  const [myProfile, setMyProfile] = useState({
    ...
    connection: user?.connection || 0,
    interested: user?.interested || 0,
    completion: user?.completion || 0,
  });

  // keep local state in sync when Redux user changes
  useEffect(() => {
    setMyProfile({
      _id: user?._id || "",
      firstName: user?.firstName || "",
  ...
  // useEffect(() => {
  //   return () => {
  //     console.log("escaping from the use effectEffect");

  //       saveToDB();
  //     setAnyChange(false);
  //   };
  // }, []);
  console.log(user?.firstName, "selector user");
  ...
  const saveBasicInfo = () => {
    const updatedProfile = {
      ...myProfile,
      firstName: editForm.firstName || myProfile.firstName,
      lastName: editForm.lastName || myProfile.lastName,
      bio: editForm.bio || myProfile.bio,
      location: editForm.location || myProfile.location,
      github: editForm.github || myProfile.github,
      profileUrl: editForm.profileUrl || myProfile.profileUrl,
    };
    console.log(myProfile, "updated profile");
  ...
  const addSkill = () => {
    if (editForm.name && editForm.level && editForm.category) {
      const updatedSkill = {
        ...myProfile,
        skill: [...myProfile.skill, { ...editForm, id: Date.now() }],
      };
      // setMyProfile((prev) => ({
      //   ...prev,
      //   skill: [...prev.skill, { ...editForm, id: Date.now() }],
      // }));
      dispatch(addUser(updatedSkill));
      setAnyChange(true);
      cancelEditing();
    }
  };
  ...
  const removeSkill = (index) => {
    ...
    const updatedSkill = {
      ...myProfile,
      skill: myProfile.skill.filter((_, i) => i !== index),
    };
    console.log(updatedSkill, "updated user");
    dispatch(addUser(updatedSkill));
    setAnyChange(true);
  };
                ...
                <div className="p-6">
                  <div className="flex justify-between items-start mb-4">
                    ...
                    {isOwnProfile && (
                      <button
                        onClick={() =>
                          startEditing("basicInfo", {
                            firstName: myProfile.firstName,
                            lastName: myProfile.lastName,
                            bio: myProfile.bio,
                            location: myProfile.location,
                            githubUrl: myProfile.github,
                  ...
                  {editingSection === "basicInfo" ? (
                    ...
                    <div className="text-center space-y-4">
                      ...
                      <div
                        ...
                        className="h-24 w-24 mx-auto rounded-full flex items-center justify-center text-white text-lg font-semibold"
                      >
                      ...
                      <div>
                        ...
                        <p className="text-gray-600">
                          {myProfile.bio || "This is default bio description."}
                        </p>
                      </div>
                    </div>
                  )}
                  ...
                  {editingSection !== "basicInfo" && (
                    <>
                      ...
                      <div className="space-y-2">
                        <a
                          href={myProfile.github || "https://github.com/"}
                          target="_blank"
                ...
                <div className="p-6">
                  ...
                  <div className="space-y-4">
                    {[
                      ...
                      const categorySkills = myProfile.skill.filter(
                          ...
                          <div className="flex flex-wrap gap-2">
                            {categorySkills.map((skill, index) => (
                              <div key={index} className="relative group">
                                <span
                                  className={`px-2 py-1 rounded-md text-sm ${getSkillLevelColor(
                                    skill.level
                                  )}`}
                                >
